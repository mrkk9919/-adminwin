...
def require_admin(request: Request) -> str:
    ...
    DEV ONLY: allow the local QA token used by the prebuilt frontend during
    local testing so the browser app can connect to admin bridge endpoints
    without a real JWT. This is intentionally gated to a single known token.
    """
    token = _extract_token(request)
    if token == "dev-token-123" or request.headers.get("X-Dev-Token") == "dev-token-123":
        return "dev-admin"
    if not token:
        raise HTTPException(
