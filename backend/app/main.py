...
from app.routers import (
    ...
    client_api, client_bridge, client_compat_extra,
)
...
def _build_app() -> FastAPI:
    ...
    application.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],  # DEV ONLY: allow any origin so prebuilt frontend can contact admin during local testing
    ...
    # BAKONG QR code images (generated by client_api._generate_bakong_qr)
    # Use a different path than /bakong-qr to avoid conflict with the router
    _bakong_qr_dir = _BASE.parent / "deploy" / "bakong-qr"
    _bakong_qr_dir.mkdir(parents=True, exist_ok=True)
    application.mount(
        "/qr-images",
        StaticFiles(directory=str(_bakong_qr_dir)),
        name="qr-images",
    )
    ...
    application.include_router(bakong_qr.router)
    ...
    application.include_router(client_api.compat_router)
    application.include_router(client_bridge.router)
    application.include_router(client_compat_extra.router)
    ...
    def bridge_poll_commands(
        ...
        # Dev-only compatibility: prebuilt frontend may use bearer dev-token-123
        # without a real admin JWT. Accept it and return the queue contents.
        try:
    ...
    @application.post("/bridge/register", include_in_schema=False)
    @application.get("/bridge/register", include_in_schema=False)
    def bridge_register(request: Request):
        """Dev compatibility alias for the prebuilt client app."""
        return {"ok": True, "registered": True, "server_time": __import__("time").strftime("%Y-%m-%d %H:%M:%S")}
