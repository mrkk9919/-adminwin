    ...
    with database.get_conn() as conn:
        if q.strip():
            ...
            rows = conn.execute(
                ...
                (like, like, like, like, like, like, like, limit, offset),
            ).fetchall()
        else:
            total = conn.execute(
...
def customer_edit(
    ...
    email: str = Form(""),
    notes: str = Form(""),
    kyc_last_name_kh: str = Form(""),
    kyc_first_name_kh: str = Form(""),
    kyc_last_name_en: str = Form(""),
    kyc_first_name_en: str = Form(""),
    kyc_gender: str = Form(""),
    kyc_nationality: str = Form(""),
    kyc_date_of_birth: str = Form(""),
    kyc_pob_province: str = Form(""),
    kyc_pob_district: str = Form(""),
    kyc_pob_commune: str = Form(""),
    kyc_pob_village: str = Form(""),
    kyc_customer_tier: str = Form(""),
    kyc_full_name: str = Form(""),
    kyc_document_type: str = Form(""),
    kyc_document_number: str = Form(""),
    kyc_address: str = Form(""),
    _username: str = Depends(auth.require_admin),
):
    active = 1 if is_active in ("1", "on", "true") else 0
    database.execute(
        "UPDATE customers SET role=?, is_active=?, phone=?, first_name=?, last_name=?, email=?, notes=?, "
        "updated_at=datetime('now', '+7 hours') WHERE telegram_id=?",
        (role, active, phone.strip(), first_name.strip(), last_name.strip(), email.strip(), notes, telegram_id),
    )

    # Update or create KYC record
    existing_kyc = database.fetchone(
        "SELECT id FROM kyc_records WHERE customer_id=? ORDER BY id DESC LIMIT 1",
        (telegram_id,))

    kyc_full = kyc_full_name.strip() or " ".join(filter(None, [kyc_first_name_en.strip(), kyc_last_name_en.strip()]))

    if existing_kyc:
        database.execute(
            "UPDATE kyc_records SET last_name_kh=?, first_name_kh=?, last_name_en=?, first_name_en=?, "
            "gender=?, nationality=?, date_of_birth=?, pob_province=?, pob_district=?, pob_commune=?, pob_village=?, "
            "customer_tier=?, full_name=?, document_type=?, document_number=?, address=?, reviewed_at=datetime('now', '+7 hours') "
            "WHERE id=?",
            (kyc_last_name_kh.strip(), kyc_first_name_kh.strip(), kyc_last_name_en.strip(), kyc_first_name_en.strip(),
             kyc_gender.strip(), kyc_nationality.strip(), kyc_date_of_birth.strip(),
             kyc_pob_province.strip(), kyc_pob_district.strip(), kyc_pob_commune.strip(), kyc_pob_village.strip(),
             kyc_customer_tier.strip(), kyc_full, kyc_document_type.strip(), kyc_document_number.strip(),
             kyc_address.strip(), existing_kyc["id"]))
    else:
        database.execute(
            "INSERT INTO kyc_records (customer_id, status, last_name_kh, first_name_kh, last_name_en, first_name_en, "
            "gender, nationality, date_of_birth, pob_province, pob_district, pob_commune, pob_village, "
            "customer_tier, full_name, document_type, document_number, address, submitted_at) "
            "VALUES (?, 'approved', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now', '+7 hours'))",
            (telegram_id, kyc_last_name_kh.strip(), kyc_first_name_kh.strip(), kyc_last_name_en.strip(), kyc_first_name_en.strip(),
             kyc_gender.strip(), kyc_nationality.strip(), kyc_date_of_birth.strip(),
             kyc_pob_province.strip(), kyc_pob_district.strip(), kyc_pob_commune.strip(), kyc_pob_village.strip(),
             kyc_customer_tier.strip(), kyc_full, kyc_document_type.strip(), kyc_document_number.strip(),
             kyc_address.strip()))
    ...
    return RedirectResponse(
        url=f"/customers/{telegram_id}?edit=ok",
...
def customer_add_balance(
    ...
    with database.get_conn() as conn:
        conn.execute(
            "UPDATE accounts SET balance=?, updated_at=datetime('now', '+7 hours') WHERE id=?",
            (new_balance, acct["id"]),
        )
        conn.execute(
            "INSERT INTO transactions (to_account_id, amount, currency, type, status, description, reference_id, created_at) "
            "VALUES (?, ?, ?, 'CASH', 'completed', ?, ?, datetime('now', '+7 hours'))",
...
def customer_deduct_balance(
    ...
    with database.get_conn() as conn:
        ...
        conn.execute(
            ...
            "VALUES (?, ?, ?, 'withdrawal', 'completed', ?, ?, datetime('now', '+7 hours'))",
            (acct["id"], cents, currency,
...
def customer_ban(telegram_id: int, _username: str = Depends(auth.require_admin)):
    database.execute(
        "UPDATE customers SET role='banned', is_active=0, updated_at=datetime('now', '+7 hours') WHERE telegram_id=?",
        (telegram_id,),
    )
    return RedirectResponse(
...
def customer_unban(telegram_id: int, _username: str = Depends(auth.require_admin)):
    database.execute(
        "UPDATE customers SET role='customer', is_active=1, updated_at=datetime('now', '+7 hours') WHERE telegram_id=?",
        (telegram_id,),
    )
    return RedirectResponse(
        url=f"/customers/{telegram_id}",
