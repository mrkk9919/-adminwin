    ...
    with database.get_conn() as conn:
        ...
        rows = conn.execute(
            ...
            SELECT
              ...
              (SELECT a.account_number FROM accounts a WHERE a.customer_id = c.telegram_id AND a.currency = 'USD' LIMIT 1) AS usd_account,
              (SELECT a.account_number FROM accounts a WHERE a.customer_id = c.telegram_id AND a.currency = 'KHR' LIMIT 1) AS khr_account
            FROM customers c
            WHERE {where}
...
def account_edit(
    ...
    role = (role or "customer").strip() or "customer"
    database.execute(
...
def account_freeze(account_id: int, _username: str = Depends(auth.require_admin)):
    database.execute(
        "UPDATE accounts SET status = 'frozen', updated_at = datetime('now', '+7 hours') WHERE id = ?",
        (account_id,),
    )
...
def account_unfreeze(account_id: int, _username: str = Depends(auth.require_admin)):
    database.execute(
        "UPDATE accounts SET status = 'active', updated_at = datetime('now', '+7 hours') WHERE id = ?",
        (account_id,),
    )
    return RedirectResponse(url=f"/accounts", status_code=status.HTTP_303_SEE_OTHER)
...
def edit_account(
    ...
    """Edit an existing account: number, balance, status, type."""
    ...
    account_status = (account_status or "active").strip() or "active"
    account_type = (account_type or "wallet").strip() or "wallet"

    try:
        database.execute(
            "UPDATE accounts SET account_number = ?, balance = ?, status = ?, type = ?, "
            "updated_at = datetime('now', '+7 hours') WHERE id = ?",
            (account_number, balance_value, account_status, account_type, account_id),
        )
    except sqlite3.IntegrityError as exc:
        raise HTTPException(
            status_code=400,
    ...
    return RedirectResponse(
        url=f"/accounts/{account['customer_id']}",
        status_code=status.HTTP_303_SEE_OTHER,
    )
