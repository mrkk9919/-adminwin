...
def kyc_approve(
    ...
    with database.get_conn() as conn:
        ...
        if not record:
            raise HTTPException(status_code=404, detail="KYC record not found")
        conn.execute(
            "UPDATE kyc_records SET status = 'approved', reviewed_at = CURRENT_TIMESTAMP, "
            "reviewed_by = ? WHERE id = ?",
            (reviewer, kyc_id),
        )
        conn.execute(
            "UPDATE customers SET kyc_status = 'approved', updated_at = CURRENT_TIMESTAMP "
            "WHERE telegram_id = ?",
            (record["customer_id"],),
        )
...
def kyc_reject(
    ...
    with database.get_conn() as conn:
        ...
        if not record:
            raise HTTPException(status_code=404, detail="KYC record not found")
        conn.execute(
            "UPDATE kyc_records SET status = 'rejected', reviewed_at = CURRENT_TIMESTAMP, "
            "reviewed_by = ?, rejection_reason = ? WHERE id = ?",
            (reviewer, reason, kyc_id),
        )
        conn.execute(
            "UPDATE customers SET kyc_status = 'rejected', updated_at = CURRENT_TIMESTAMP "
            "WHERE telegram_id = ?",
            (record["customer_id"],),
        )
    return RedirectResponse(url=f"/kyc/{kyc_id}", status_code=status.HTTP_303_SEE_OTHER)
