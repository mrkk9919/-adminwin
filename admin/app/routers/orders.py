...
def hash_lookup_page(
    ...
    """Hash Lookup page — enter a hash to see transfer details."""
    order = None
    if q.strip():
        row = database.fetchone(
            "SELECT o.*, c.username, c.first_name, c.last_name "
...
def api_hash_lookup(
    ...
    """JSON API — look up a single order by hash."""
    q = q.strip()
    if not q:
    ...
    if not row:
        return {"ok": True, "found": False}
    return {"ok": True, "found": True, "order": dict(row)}
...
def order_edit(
    ...
    database.execute(
        ...
        "bank=?, receiver=?, tx_date=?, tx_id=?, notes=?, updated_at=CURRENT_TIMESTAMP "
        "WHERE hash=?",
